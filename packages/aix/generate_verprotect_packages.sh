...
build_package() {
  ...
  source_code="http://api.github.com/repos/verprotect/verprotect/tarball/${reference}"

  rm -f verprotect.tar.gz && curl -L ${source_code} -k -o verprotect.tar.gz -s
  rm -rf verprotect-verprotect-* verprotect-agent-*
  extracted_directory=$(gunzip -c verprotect.tar.gz | tar -xvf - | tail -n 1 | cut -d' ' -f2 | cut -d'/' -f1)
  verprotect_version=$(cat ${extracted_directory}/src/VERSION | cut -d'v' -f2)
  cp -pr ${extracted_directory} verprotect-agent-${verprotect_version}
  ...
  package_name=verprotect-agent-${verprotect_version}
  tar cf ${package_name}.tar ${package_name} && gzip ${package_name}.tar
  mv ${package_name}.tar.gz ${rpm_build_dir}/SOURCES/

  cp ${current_path}/SPECS/verprotect-agent-aix.spec ${rpm_build_dir}/SPECS/${package_name}-aix.spec
  ...
  --define "_init_scripts ${init_scripts}" --define "_sysconfdir ${sysconfdir}" --define "_version ${verprotect_version}" \
  ...
  find /etc/ -name "*verprotect*" -exec rm {} \;
