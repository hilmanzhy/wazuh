...
# Verprotect HP-UX Package builder.
...
source_directory=/verprotect-sources
configuration_file="${source_directory}/etc/preloaded-vars.conf"
target_dir="${current_path}/output/"
verprotect_version=""
verprotect_revision="1"
depot_path=""
control_binary=""
compute_checksums="no"

# Needed variables to build Verprotect with custom GCC and cmake
...
build_environment() {
    ...
    # Resizing partitions for Site Ox boxes (used by Verprotect team)
    ...
    curl -k -SO http://packages.verprotect.com/utils/gcc/gcc_9.4_HPUX_build.tar.gz
    ...
    cd ${build_tools_path}
    curl -k -SO http://packages.verprotect.com/utils/cmake/cmake_3.22.2_HPUX_build.tar.gz
...
compute_version_revision() {
    verprotect_version=$(cat ${source_directory}/src/VERSION | cut -d "-" -f1 | cut -c 2-)

    echo ${verprotect_version} > /tmp/VERSION
    echo ${verprotect_revision} > /tmp/REVISION
...
check_version() {
    verprotect_version=`cat ${source_directory}/src/VERSION`
...
compile() {
    echo "Compiling code"
    # Compile and install verprotect
    cd ${source_directory}/src
    config
    check_version
    gmake deps RESOURCES_URL=http://packages.verprotect.com/deps/${deps_version} TARGET=agent
...
create_package() {
    ...
    verprotect_version=`echo "${verprotect_version}" | cut -d v -f 2`
    pkg_tar_file="verprotect-agent-${verprotect_version}-${verprotect_revision}-hpux-11v3-ia64.tar"
    tar cvpf ${target_dir}/${pkg_tar_file} ${install_path} /sbin/init.d/verprotect-agent /sbin/rc2.d/S97verprotect-agent /sbin/rc3.d/S97verprotect-agent
...
clean() {
    ...
    find /sbin -name "*verprotect-agent*" -exec rm {} \;
    userdel verprotect
    groupdel verprotect
...
show_help() {
    ...
    echo "    -r, --revision Revision to naming Verprotect agent package."
