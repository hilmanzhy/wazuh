#!/bin/bash
set -e
# Program to build and package OSX verprotect-agent
...
STARTUP_SCRIPT_PATH="/Library/StartupItems/VERPROTECT/VERPROTECT"
...
function clean_and_exit() {
    ...
    if [ -z "$BRANCH_TAG" ]; then
        make -C $VERPROTECT_PATH/src clean clean-deps
    fi
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in Verprotect's installation. This also includes library files.
...
function prepare_building_folder() {
    ...
    version="$1"
    pkg_final_name="$2"
    build_info_file="${VERPROTECT_PACKAGES_PATH}/specs/build-info.json"
    preinstall_script="${VERPROTECT_PACKAGES_PATH}/package_files/preinstall.sh"
    postinstall_script="${VERPROTECT_PACKAGES_PATH}/package_files/postinstall.sh"
    packaged_directory=$CURRENT_PATH/verprotect-agent/payload

    if [ -d "$CURRENT_PATH/verprotect-agent" ]; then

        echo "\nThe verprotect agent building directory is present on this machine."
        echo "Removing it from the system."

        rm -rf $CURRENT_PATH/verprotect-agent
    fi

    munkipkg --create --json $CURRENT_PATH/verprotect-agent

    cp -f $build_info_file $CURRENT_PATH/verprotect-agent/

    sed -i '' "s|VERSION|$version|g" $CURRENT_PATH/verprotect-agent/$(basename $build_info_file)
    sed -i '' "s|PACKAGE_NAME|$pkg_final_name|g" $CURRENT_PATH/verprotect-agent/$(basename $build_info_file)

    cp $preinstall_script $CURRENT_PATH/verprotect-agent/scripts/preinstall
    cp $postinstall_script $CURRENT_PATH/verprotect-agent/scripts/postinstall

    sed -i '' "s|PACKAGE_ARCH|$ARCH|g" $CURRENT_PATH/verprotect-agent/scripts/preinstall
...
function build_package() {
    ...
    if [ -n "$BRANCH_TAG" ]; then
        ...
        VERPROTECT_PATH="${SOURCES_DIRECTORY}/verprotect"
        git clone --depth=1 -b ${BRANCH_TAG} ${VERPROTECT_SOURCE_REPOSITORY} "${VERPROTECT_PATH}"
    else
        VERPROTECT_PATH="${CURRENT_PATH}/../.."
    fi
    short_commit_hash="$(cd "${VERPROTECT_PATH}" && git rev-parse --short HEAD)"

    export CONFIG="${VERPROTECT_PATH}/etc/preloaded-vars.conf"
    VERPROTECT_PACKAGES_PATH="${VERPROTECT_PATH}/packages/macos"
    ENTITLEMENTS_PATH="${VERPROTECT_PACKAGES_PATH}/entitlements.plist"

    VERSION=$(cat ${VERPROTECT_PATH}/src/VERSION | cut -d "-" -f1 | cut -c 2-)

    # Define output package name
    if [ $IS_STAGE == "no" ]; then
        pkg_name="verprotect-agent_${VERSION}-${REVISION}_${ARCH}_${short_commit_hash}"
    else
        pkg_name="verprotect-agent-${VERSION}-${REVISION}.${ARCH}"
    fi

    if [ -d "${INSTALLATION_PATH}" ]; then

        echo "\nThe verprotect agent is already installed on this machine."
        echo "Removing it from the system."

        ${CURRENT_PATH}/uninstall.sh
    fi

    ${VERPROTECT_PACKAGES_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${VERPROTECT_PATH}" ${JOBS} ${DEBUG} ${MAKE_COMPILATION}
    ...
    # create package
    if munkipkg $CURRENT_PATH/verprotect-agent ; then
        echo "The verprotect agent package for macOS has been successfully built."
        mv $CURRENT_PATH/verprotect-agent/build/* $DESTINATION/
        symbols_pkg_name="${pkg_name}_debug_symbols"
        cp -R "${VERPROTECT_PATH}/src/symbols"  "${DESTINATION}"
