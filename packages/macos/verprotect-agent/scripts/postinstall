...
UPGRADE_FILE_FLAG="${DIR}/VERPROTECT_PKG_UPGRADE"
...
if [ -z "${upgrade}" ]; then
    echo "Generating Verprotect configuration for a fresh installation."
    ...
    if [ -f "${INSTALLATION_SCRIPTS_DIR}/gen_ossec.sh" ]; then
        ...
        chown root:verprotect ${DIR}/etc/ossec.conf
...
if [[ $(dscl . -read /Groups/ossec) ]]; then
    echo "Changing group from Ossec to Verprotect"
    find ${DIR}/ -group ossec -user root -exec chown root:verprotect {} \ > /dev/null 2>&1 || true
    if [[ $(dscl . -read /Users/ossec) ]]; then
        echo "Changing user from Ossec to Verprotect"
        find ${DIR}/ -group ossec -user ossec -exec chown verprotect:verprotect {} \ > /dev/null 2>&1 || true
...
if [ -n "${upgrade}" ] && [ -n "${restart}" ]; then
    echo "Restarting Verprotect..."
    ${DIR}/bin/verprotect-control restart
fi
