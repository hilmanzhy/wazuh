...
verprotect_branch="$(echo "$2" | cut -d "/" -f2)"
...
clone(){
    cd ${CURRENT_PATH}
    git clone --depth=1 $REPOSITORY -b $verprotect_branch ${SOURCE} || return 1
    cd $SOURCE
    compute_version_revision
...
package(){
    cd ${CURRENT_PATH}
    find ${install_path} | awk 'length > 0' > "verprotect-agent_$VERSION.list"
    ...
    echo "i pkginfo=pkginfo" > "verprotect-agent_$VERSION.proto"
    echo "i checkinstall=checkinstall.sh" >> "verprotect-agent_$VERSION.proto"
    echo "i preinstall=preinstall.sh" >> "verprotect-agent_$VERSION.proto"
    echo "i postinstall=postinstall.sh" >> "verprotect-agent_$VERSION.proto"
    echo "i preremove=preremove.sh" >> "verprotect-agent_$VERSION.proto"
    echo "i postremove=postremove.sh" >> "verprotect-agent_$VERSION.proto"
    echo "f none /etc/init.d/verprotect-agent  0755 root root" >> "verprotect-agent_$VERSION.proto"
    echo "s none /etc/rc2.d/S97verprotect-agent=/etc/init.d/verprotect-agent" >> "verprotect-agent_$VERSION.proto"
    echo "s none /etc/rc3.d/S97verprotect-agent=/etc/init.d/verprotect-agent" >> "verprotect-agent_$VERSION.proto"
    cat "verprotect-agent_$VERSION.list" | pkgproto >> "verprotect-agent_$VERSION.proto"

    echo $VERSION
    pkgmk -o -r / -d . -f "verprotect-agent_$VERSION.proto"
    pkg_name="verprotect-agent_$VERSION-sol10-$ARCH.pkg"
    pkgtrans -s ${CURRENT_PATH} "${pkg_name}" verprotect-agent
...
clean(){
    ...
    cd ${CURRENT_PATH}
    rm -rf ${SOURCE}
    rm -rf verprotect-agent verprotect *.list *proto
    rm -f *.new
...
build(){
    ...
    groupadd verprotect
    useradd -g verprotect verprotect
    installation
    package
}
