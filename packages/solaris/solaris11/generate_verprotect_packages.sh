...
set_control_binary() {
    if [ -e ${SOURCE}/src/VERSION ]; then
        verprotect_version=`cat ${SOURCE}/src/VERSION`
        number_version=`echo "${verprotect_version}" | cut -d v -f 2`
...
compute_version_revision() {
    verprotect_version=$(cat ${SOURCE}/src/VERSION | cut -d "-" -f1 | cut -c 2-)
    revision="$(cat ${SOURCE}/src/REVISION)"

    echo $verprotect_version > /tmp/VERSION
...
download_source() {
    ...
    if [[ "${verprotect_branch}" != "trunk" ]] || [[ "${verprotect_branch}" != "master" ]]; then
        cd $SOURCE
        git checkout $verprotect_branch
    fi
    cd ${current_path}
    compute_version_revision
}
...
#Compile and install verprotect-agent
compile() {
...
create_package() {
    ...
    if [ $(echo $VERSION | grep "v") ]; then
        ...
        sed "s/<VERSION>/$ver/" ${current_path}/verprotect-agent.mog-template > ${current_path}/verprotect-agent.mog-aux
    else
        sed "s/<VERSION>/$VERSION/" ${current_path}/verprotect-agent.mog-template > ${current_path}/verprotect-agent.mog-aux
    fi
    sed "s/<TAG>/$VERSION/" ${current_path}/verprotect-agent.mog-aux > ${current_path}/verprotect-agent.mog

    echo "Building the package verprotect-agent_$VERSION-sol11-${arch}.p5p"

    set_control_binary


    # Package generation process
    pkgsend generate ${install_path} | pkgfmt > verprotect-agent.p5m.1
    ...
    python solaris_fix.py -t SPECS/template_agent.json -p verprotect-agent.p5m.1 # Fix p5m.1 file
    mv verprotect-agent.p5m.1.aux.fixed verprotect-agent.p5m.1
    ...
    for file in etc/ossec.conf etc/local_internal_options.conf etc/client.keys; do
        sed "s:file $file.*:& preserve=install-only:"  verprotect-agent.p5m.1 > verprotect-agent.p5m.1.aux_sed
        mv verprotect-agent.p5m.1.aux_sed verprotect-agent.p5m.1
    done
    # Add service files
    echo "file smf_manifest.xml path=lib/svc/manifest/site/post-install.xml owner=root group=sys mode=0744 restart_fmri=svc:/system/manifest-import:default" >> verprotect-agent.p5m.1
    echo "dir  path=var/ossec/installation_scripts owner=root group=bin mode=0755" >> verprotect-agent.p5m.1
    echo "file postinstall.sh path=var/ossec/installation_scripts/postinstall.sh owner=root group=bin mode=0744" >> verprotect-agent.p5m.1
    echo "file verprotect-agent path=etc/init.d/verprotect-agent owner=root group=sys mode=0744" >> verprotect-agent.p5m.1
    echo "file S97verprotect-agent path=etc/rc2.d/S97verprotect-agent owner=root group=sys mode=0744" >> verprotect-agent.p5m.1
    echo "file S97verprotect-agent path=etc/rc3.d/S97verprotect-agent owner=root group=sys mode=0744" >> verprotect-agent.p5m.1

    # Add user and group verprotect
    echo "group groupname=verprotect" >> verprotect-agent.p5m.1
    echo "user username=verprotect group=verprotect" >> verprotect-agent.p5m.1

    # Necessary to upgrade from < 4.3 versions
    echo "group groupname=ossec" >> verprotect-agent.p5m.1
    echo "user username=ossec group=ossec" >> verprotect-agent.p5m.1

    pkgmogrify -DARCH=`uname -p` verprotect-agent.p5m.1 verprotect-agent.mog | pkgfmt > verprotect-agent.p5m.2
    pkgsend -s http://localhost:9001 publish -d ${install_path} -d /etc/init.d -d /etc/rc2.d -d /etc/rc3.d -d ${current_path} verprotect-agent.p5m.2 > pack
    package=`cat pack | grep verprotect | cut -c 13-` # This extracts the name of the package generated in the previous step
    rm -f *.p5p
    pkg_name="verprotect-agent_$VERSION-sol11-${arch}.p5p"
...
create_repo() {
    zfs create rpool/verprotect
    zfs set mountpoint=/verprotect rpool/verprotect
    pkgrepo create /verprotect
    ls /verprotect
    pkgrepo -s /verprotect set publisher/prefix=verprotect
    svccfg -s application/pkg/server setprop \ pkg/inst_root=/verprotect
...
uninstall() {
    ...
    rm -f `find /etc | grep verprotect`
    rm -f /etc/rc3.d/S97verprotect-agent
    rm -f /etc/rc2.d/S97verprotect-agent
}
...
clean() {
    ...
    umount /verprotect
    zfs destroy rpool/verprotect
    rm -rf /verprotect
    rm -rf $SOURCE/verprotect
    rm -f verprotect-agent.p5m*
    rm -f verprotect-agent.mog
    rm -f verprotect-agent.mog-aux
    rm -f pack
}
...
show_help() {
    ...
    echo -e "                Select Git branch or tag e.g. ${verprotect_branch}."
    echo -e ""
    echo -e "        -c, --checksum"
