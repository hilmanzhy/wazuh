...
param (
    [string]$MSI_NAME = "verprotect-agent.msi",
    [string]$SIGN = "no",
...
if(($help.isPresent)) {
    "
    This tool can be used to generate the Windows Verprotect agent msi package.

    PARAMETERS TO BUILD VERPROTECT-AGENT MSI (OPTIONALS):
    ...
    USAGE:
        ...
        * VERPROTECT:
          $ ./generate_verprotect_msi.ps1  -MSI_NAME {{ NAME }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}
            Build a devel msi:    $ ./generate_verprotect_msi.ps1 -MSI_NAME verprotect-agent_4.9.0-0_windows_0ceb378.msi -SIGN no
            Build a prod msi:     $ ./generate_verprotect_msi.ps1 -MSI_NAME verprotect-agent-4.9.0-1.msi -SIGN yes
    "
    Exit
}
...
function BuildVerprotectMsi(){
    ...
    if($SIGN -eq "yes"){
        ...
        $filesToSign = @(
            ...
            "..\verprotect_modules\syscollector\build\bin\syscollector.dll",
    ...
    & $CANDLE_EXE -nologo .\verprotect-installer.wxs -out "verprotect-installer.wixobj" -ext WixUtilExtension -ext WixUiExtension
    & $LIGHT_EXE ".\verprotect-installer.wixobj" -out $MSI_NAME -ext WixUtilExtension -ext WixUiExtension
...
function ExtractDebugSymbols(){
	...
	$exeFiles +=  Get-ChildItem -Filter "verprotect_modules\syscollector\build\bin\syscollector.dll"
