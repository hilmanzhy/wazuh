# Integration Tests
## Index
1. [Compile Verprotect](#compile-verprotect)
...
Navigate into `verprotect/src/dbsync/` and run the following commands:
```
mkdir build
cd build
cmake -DEXTERNAL_LIB=~/path/to/verprotect/src/external/ -DCMAKE_BUILD_TYPE=Debug -DUNIT_TEST=ON ..
cmake --build .
```
