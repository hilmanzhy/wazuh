...
/* Install the VERPROTECT-HIDS agent service */
int InstallService(char *path);

/* Uninstall the VERPROTECT-HIDS agent service */
int UninstallService();

/* Check if the VERPROTECT-HIDS agent service is running
 * Returns 1 on success (running) or 0 if not running
 */
int CheckServiceRunning();

/* Start VERPROTECT-HIDS service */
int os_start_service();

/* Stop VERPROTECT-HIDS service */
int os_stop_service();
