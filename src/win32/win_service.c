...
static LPTSTR g_lpszServiceName        = "VerprotectSvc";
static LPTSTR g_lpszServiceDisplayName = "Verprotect";
static LPTSTR g_lpszServiceDescription = "Verprotect Windows Agent";
...
int os_stop_service()
{
    ...
    * newer instances of verprotect-agent.
    */
    ...
    if (ossecServiceStatusHandle) {
        switch (dwOpcode) {
            case SERVICE_CONTROL_STOP:
                ...
                // Kill children processes spawned by modules, only in verprotect-agent
                wm_kill_children();
                stop_wmodules();
